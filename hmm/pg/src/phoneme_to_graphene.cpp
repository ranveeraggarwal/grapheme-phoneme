...
int main(){
	...
	while(1){
		...
		int temp3 = int_map(g);
		//cout<<p1<<" "<<temp1<<" "<<p2<<" "<<temp2<<" "<<g<<" "<<temp3<<endl;
		p[temp1][temp2][temp3] = prob;
	}

	double seqscore[27][20];
	for(int i=0; i<27; i++){
	...
	while(true){
		...
		if(s=='-')break;
			...
			seqscore[temp][0] = initprob;
		}
	}

	while(true){
		string val;
		getline(cin, val);	// graphene
		if (val == "-") break;
		val = str_toupper(val);
		...
		for(int z=1; z<t; z++){
			for(int q=0; q<27; q++){
				...
				for(int r=0; r<27; r++){
					...
					if(maxnum < seqscore[r][z-1]*p[r][q][int_map(tokens[z-1])]){
						maxnum = seqscore[r][z-1]*p[r][q][int_map(tokens[z-1])];
						bestindex = r;
					}
		...
		for(int i=0; i<t-1; i++) cout << (char)(c[i] + 'A');
		cout<<endl;
	}
